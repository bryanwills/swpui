...
pub fn render(app: &mut App, frame: &mut Frame, area: Rect) {
    ...
    let Some(fm) = app.results.get(app.selected_file()) else {
        ...
        render_message(frame, inner, "Select a file", Style::default().dim());
        return;
    };
    ...
    if let Some(err) = app.preview_error.get(&path) {
        ...
        let msg = err.clone();
        frame.render_widget(block, area);
        render_message(frame, inner, &msg, Style::default().fg(Color::Red));
        return;
    }
    ...
    let Some(preview) = app.preview_data.get(&path).cloned() else {
        ...
        render_message(frame, inner, msg, Style::default().dim());
        return;
    };
...
fn render_message(frame: &mut Frame, inner: Rect, msg: &str, style: Style) {
...
///
/// Lays out as `[indicator][line-number-column][space]`, where the indicator is `>` for the
/// matched line(s) of the selected match and a blank otherwise. The line-number column is
/// right-aligned to the largest line number that can appear in the preview, so all rows align.
struct Gutter {
    line_nb_width: usize,
}
...
impl Gutter {
    ...
    /// Gutter for a matched line: blue/cyan line number, optional `>` indicator when selected.
    ///
    /// `line_number` is `None` for `+` replacement lines that don't correspond to an original
    /// source line.
    ...
    /// Effective `max_width` to pass to [`TruncatedLine::new`] so that the gutter and content
    /// together fit within `inner_width`.
    ///
    /// `TruncatedLine` reserves 1 col internally for a leading margin, so we only need to
    /// subtract the rest of the gutter (line-number column + trailing space).
...
impl<'a> PreviewBuilder<'a> {
    ...
    fn layout(&self) -> (usize, Range<usize>) {
    ...
    fn build(&self, visible_range: Range<usize>) -> Vec<Line<'static>> {
