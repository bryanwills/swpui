...
#[derive(Debug, Clone)]
pub struct FormattedPath(String);
...
impl From<FormattedPath> for Cow<'_, str> {
    fn from(value: FormattedPath) -> Self {
        Cow::Owned(value.0)
    }
}

impl From<FormattedPath> for Text<'_> {
    fn from(value: FormattedPath) -> Self {
        Text::raw(value)
    }
}
...
impl FormattedPath {
    ...
    /// display columns.
    ///
    /// The suffix is the number of active and total matches within the file: " (3/4)"
    ///
    ...
    /// 5. Ellipsis in the basename
    /// 6. Right-aligned truncation
    #[must_use]
    pub fn new(rel: impl AsRef<Path>, suffix: &str, max_width: usize) -> Self {
        let rel = rel.as_ref();
        ...
        let Some(filename) = dirs.pop() else {
            return Self(suffix.into());
        };

        // full path
        let res = Self::join_path(&dirs, &filename);
        if Self::fits(&res, suffix, max_width) {
            return Self::with_suffix(&res, suffix);
        }

        // abbreviate directory segments to 3, then 2, then 1 characters
        for dir_size in (1..=3).rev() {
            for dir in &mut dirs {
                Self::truncate_chars(dir, dir_size);
            }
            let path = Self::join_path(&dirs, &filename);
            if Self::fits(&path, suffix, max_width) {
                return Self::with_suffix(&path, suffix);
            }
        }

        // ellipsis in the file stem
        let (stem, ext) = Self::split_stem_ext(&filename);
        let dir_prefix = Self::dir_prefix_string(&dirs);
        ...
        if stem_chars.len() > 3 {
            ...
            if max_width > overhead && max_width - overhead < stem_width {
                ...
                if start_n >= 1 && end_n >= 1 {
                    ...
                    return Self(format!("{dir_prefix}{start}\u{2026}{end}{ext}{suffix}"));
                }
            }
        }
        ...
        Self::truncate_left(&compact, compact_w - max_width)
    }
    ...
    fn with_suffix(path: &str, suffix: &str) -> Self {
        Self(format!("{path}{suffix}"))
    }

    fn truncate_left(s: &str, excess_cols: usize) -> Self {
        let mut removed = 0;
...
mod tests {
    ...
    fn full_path_when_it_fits() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (3/5)", 50).as_str(),
            "src/main.rs (3/5)"
        );
    }
    ...
    fn dirs_abbreviated_to_three_chars() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 30).as_str(),
    ...
    fn dirs_abbreviated_to_two_chars() {
        let path = Path::new("src/components/widgets/MyFile.rs");
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 26).as_str(),
    ...
    fn dirs_abbreviated_to_one_char() {
        let path = Path::new("src/components/widgets/MyFile.rs");
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 21).as_str(),
            "s/c/w/MyFile.rs (1/2)"
        );
    }
    ...
    fn ellipsis_in_stem_minimal() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 18).as_str(),
    ...
    fn ellipsis_in_stem_with_more_chars() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 20).as_str(),
    ...
    fn right_aligned_truncation() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/2)", 15).as_str(),
            "/w/M\u{2026}e.rs (1/2)"
        );
    }
    ...
    fn no_directories() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (2/3)", 20).as_str(),
            "README.md (2/3)"
        );
    }
    ...
    fn no_directories_with_ellipsis() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (2/3)", 13).as_str(),
            "RE\u{2026}E.md (2/3)"
        );
    }
    ...
    fn no_extension() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/1)", 17).as_str(),
            "sr/Makefile (1/1)"
        );
    }
    ...
    fn no_extension_with_ellipsis() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/1)", 13).as_str(),
            "s/Ma\u{2026}le (1/1)"
        );
    }
    ...
    fn short_stem_skips_ellipsis() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/1)", 10).as_str(),
            "a.rs (1/1)"
        );
    }
    ...
    fn short_dirs_not_over_abbreviated() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/1)", 25).as_str(),
            "a/b/MyFile.rs (1/1)"
        );
    }
    ...
    fn very_narrow_truncation() {
        ...
        assert_eq!(FormattedPath::new(path, " (1/2)", 5).as_str(), "(1/2)");
    }

    #[test]
    fn empty_path_returns_suffix() {
        let path = Path::new("");
        assert_eq!(FormattedPath::new(path, " (0/0)", 20).as_str(), " (0/0)");
    }
    ...
    fn dotfile_treated_as_extensionless() {
        ...
        assert_eq!(
            FormattedPath::new(path, " (1/1)", 20).as_str(),
            ".gitignore (1/1)"
        );
    }
